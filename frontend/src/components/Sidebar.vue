<template>
  <div>
    ...
    <aside
      :class="[
        'fixed inset-y-0 left-0 z-40 w-[292px] border-r border-slate-200 bg-white shadow-[0_12px_40px_-24px_rgba(15,23,42,0.25)] transition-transform duration-200 ease-out lg:static lg:translate-x-0 lg:shadow-none',
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-5">
          <ul class="space-y-1">
            <li v-for="item in primaryNav" :key="item.to">
              <RouterLink custom :to="item.to" v-slot="{ href, navigate, isActive }">
                <a
                  :href="href"
